...
async function handleRegister() {
  ...
  loading.value = true
  try {
    await store.dispatch('register', { email: email.value, password: password.value, name: name.value, role: role.value })
    router.push('/')
  } catch (e) {
...
async function handleGoogle() {
  ...
  loading.value = true
  try {
    await store.dispatch('loginWithGoogle', { role: role.value })
    router.push('/')
  } catch (e) {
    error.value = parseFirebaseError(e)
  } finally {
    ...
    <div class="w-full max-w-md">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-stone-200 p-8">
        <form @submit.prevent="handleRegister" class="space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-stone-700 mb-2">Daftar sebagai</label>
            <div class="flex gap-3">
              <label
                class="flex-1 flex items-center gap-2 px-4 py-3 border-2 rounded-xl cursor-pointer transition-all"
                :class="role === 'user'
                  ? 'border-brand-500 bg-brand-50 text-brand-700'
                  : 'border-stone-200 bg-white text-stone-600 hover:border-stone-300'"
              >
                <input type="radio" v-model="role" value="user" class="sr-only" />
                <svg class="w-5 h-5" :class="role === 'user' ? 'text-brand-600' : 'text-stone-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              >
                ...
                <svg class="w-5 h-5" :class="role === 'seller' ? 'text-brand-600' : 'text-stone-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mt-8 flex items-center gap-3">
          ...
          <span class="text-stone-400 text-xs uppercase tracking-wider font-medium">atau</span>
          <hr class="flex-1 border-stone-200" />
        </div>
